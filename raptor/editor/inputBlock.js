...
inputBlock.prototype = {
    ...
    render: function(){
        ...
        ctx.beginPath();
        ctx.moveTo(this.x-10, this.y + 5); // -> 0, 5
        ctx.lineTo(this.x + 20-3, this.y + 5); // -> 20, 5
        ctx.lineTo(this.x + 5, this.y); // -> 10,0
        ctx.lineTo(this.x + 20-3, this.y+5); // -> 20, 5
        ctx.lineTo(this.x + 5, this.y +10); // -> 10, 10
        ctx.stroke();
    ...
    neighbor: function(d){
        ...
        for(e=0; e<this.points.length; e++){
            if(this.points[e].direction == d){
                for(f=0; f<lines.length; f++){
                    if(lines[f].a == this.points[e]){ //če najdeš točko
                        point = lines[f].b;
                    }
                    if(lines[f].b == this.points[e]){ //če najdeš točko
                        point = lines[f].a;
                    }
                }
            }
        ...
        for(e=0; e<program.length; e++){
            for(f=0; f<program[e].points.length; f++){
                if(program[e].points[f] == point){ //če najdeš točko
                    return program[e];
                }
            }
        }
